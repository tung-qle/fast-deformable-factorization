...
try:
    ...
    found_einops = False
    warn("Did not find PyTorch, therefore use NumPy.")
...
def random_generate(param, seed=None):
    """
    ...
    if found_pytorch:
        if seed:
            ...
            gen = None
        if len(param) == 6:
            return torch.rand(param[0], param[3],
                              param[1] * param[4], param[2] * param[5], generator=gen)
        return torch.rand(param[0], param[3], param[1], param[2], generator=gen)
    else:
        np.random.seed(seed if seed else 1)
        if len(param) == 6:
            return np.random.rand(param[0], param[3],
...
def twiddle_to_dense(twiddle):
    ...
    if found_pytorch:
        ...
        t = twiddle.reshape(a * d, b, c)  # (a * d, b, c)
        output = output.reshape(a, c, d, n).swapaxes(1, 2).reshape(a * d, c, n)  # (a * d, c, n)
        output = np.einsum('ijk,ikl->ijl', t, output)  # (a * d, b, n)
...
if __name__ == "__main__":
    ...
    param2 = [2, 2, 2, 1, 2, 1]
    if found_pytorch:
        twiddle1 = torch.rand(param1[0], param1[3],
                              param1[1] * param1[4], param1[2] * param1[5])
        twiddle2 = torch.rand(param2[0], param2[3],
                              param2[1] * param2[4], param2[2] * param2[5])
    else:
        twiddle1 = np.random.rand(param1[0], param1[3],
                                  param1[1] * param1[4], param1[2] * param1[5])
        twiddle2 = np.random.rand(param2[0], param2[3],
                                  param2[1] * param2[4], param2[2] * param2[5])
