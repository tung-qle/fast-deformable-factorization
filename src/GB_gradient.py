...
class GBGradient():
    ...
    def approximate_matrix(self, matrix, lr = 0.05, optimizer = "Adam", momentum = 0.9, epochs = 100, epsilon = 1e-10, device = 'cpu', num_iter_refined = 20):
        ...
        for index in range(epochs):
            optalg.zero_grad()
            output = loss(self.twiddle_to_matrix(device = device), matrix)
            output.backward()
            optalg.step()
            
            with torch.no_grad():
                error.append(torch.linalg.norm(self.twiddle_to_matrix(device = device) - matrix) / torch.linalg.norm(matrix))
            end = time.time()
        ...
        return error + refined_loss, running_time + refined_time
