...
def parse_arg():
    ...
    parser.add_argument("--save-dir", type=Path, default="./results/hierarchical_comp_new")
...
if __name__ == "__main__":
    ...
    noise_level = (torch.linalg.norm(noise_matrix) / torch.linalg.norm(matrix)).item()
    ...
    error = (torch.norm(matrix - operator.densification(factor_list, min_param)) / torch.norm(matrix)).item()
    ...
    df.to_csv(save_dir / f"n_factors={n_factors}-rank={rank}-noise={noise}-matrix_size={matrix_size}-seed={seed}-orthonormalize={orthonormalize}-hierarchical-order={hierarchical_order}.csv")
    print(df)
